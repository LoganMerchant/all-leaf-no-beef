  ...
  connection.query(
    ...
    (error, results) => {
      if (error) {
        ...
        if (results.length > 0) uid = results[0].id;
      }
      connection.query(
        ...
        (error, results2) => {
          if (error) {
            ...
            if (results2.length > 0) {
              ...
              if (
                comment.topic_name == "undefined" ||
                comment.topic_name == null ||
                comment.topic_name == ""
              ) {
                res.send({
                ...
                connection.query(
                  ...
                  (error, results, fields) => {
                    if (error) {
                      ...
                      connection.query(
                        `UPDATE topics SET comments = comments+1 WHERE topics.id = ${results2[0].id}`,
                        (error, results, fields) => {
                          res.send({
                ...
                connection.query(
                  ...
                  (error, results3) => {
                    if (error) {
                      res.send({
                        failed:
                          "error ocurred while checking comments. Try again!",
                      });
                    } else {
                      if (results3[0].user_id == uid) {
                        connection.query(
                          `UPDATE comments SET text = "${comment.text}", updated_at= "${comment.updated_at}" WHERE comments.id = ${results3[0].id}`,
                          (error, results) => {
                            if (error) {
                              res.send({
                        ...
                        res.send({
                          failed:
                            "You can only modify comment that you create!",
                        });
                      }
                    }
                  }
...
exports.getComment = (req, res) => {
  ...
  if (topic.name == "undefined" || topic.name == null || topic.name == "") {
    ...
    connection.query(
      "SELECT * FROM comments WHERE topic_name = ?",
      [topic.name],
      (error, results) => {
        if (error) {
          res.send({
          ...
          if (results.length == 0) {
            res.send({
              failed:
                "topic with that name doesn't exist or there is no comments on that topic",
            });
          } else {
            connection.query(
  ...
  connection.query(
    ...
    (error, results) => {
      ...
      if (
        ...
        connection.query(
          `SELECT * FROM comments WHERE topic_name = "${comment.topic_name}" and user_id= ${uid}`,
          (error, results2) => {
            if (error) {
              console.log(error);
              res.send({
                failed:
                  "error ocurred while searching for topic name. Try again!",
              });
            } else {
              if (results2.length > 0) {
                connection.query(
                  `delete from comments  WHERE id = ${results2[0].id}`,
                  (error, results3) => {
                    if (error) {
                      console.log(error);
                      res.send({
                        failed:
                          "error ocurred while deleting comment. Try again!",
                      });
                    } else {
                      connection.query(
                        `UPDATE topics SET comments = comments-1 WHERE topics.name = "${comment.topic_name}"`,
                        (error, results) => {
                          if (error) {
                            res.send({
                              failed:
                                "error ocurred while modifying topic table",
                            });
                          } else {
                            res.send({
                ...
                res.send({
                  success:
                    "topic with that name doesn't exist or you don't have any comments on this topic!",
                });
              }
            }
          }
