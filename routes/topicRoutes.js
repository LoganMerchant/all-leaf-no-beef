...
exports.createTopic = (req, res) => {
  ...
  connection.query(
    ...
    (error, results) => {
      ...
      } else if (results.length > 0) user_id = results[0].id;
...
exports.modifyTopic = (req, res) => {
  ...
  connection.query(
    ...
    (error, results) => {
      ...
      } else if (results.length > 0) uid = results[0].id;
      ...
      if (topic.name == "undefined" || topic.name == null || topic.name == "") {
        res.send({
          failed:
            "We can't find topic without name. Please enter a name of topic",
        });
      } else if (
        topic.text == "undefined" ||
        topic.text == "" ||
            ...
            if (error) {
              ...
              if (results.length > 0) {
                if (results[0].user_id == uid) {
                  connection.query(
                    `UPDATE topics SET text = "${topic.text}", updated_at= "${topic.updated_at}" WHERE topics.id = ${results[0].id}`,
                    (error, results, fields) => {
                      if (error) {
                        console.log("error ocurred ", error);
                        res.send({
              ...
              (err, results2) => {
                if (err) {
                  ...
                  res.send({
                    ...
                    "posted by":
                      results2[0].first_name + " " + results2[0].last_name,
          ...
          (error, results) => {
            if (error) {
              ...
              if (results.length > 0) {
                if (results[0].user_id == uid) {
                  connection.query(
                    `delete from topics  WHERE topics.id = ${results[0].id}`,
                    (error, results) => {
                      if (error) {
                        console.log("error ocurred ", error);
                        res.send({
